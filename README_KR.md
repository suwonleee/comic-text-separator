...
- JSX 파일 내 `FONT_NAME` 변수를 수정하여 원하는 폰트 지정 가능 (기본값: NanumGothic). 지정한 폰트가 없을 경우 Arial로 자동 대체됨
- JSX 실행 시 기존 래스터 텍스트 그룹("Text Layers")은 자동 삭제되고, 편집 가능한 텍스트 그룹으로 대체됨
- 생성된 텍스트 그룹은 레이어 스택 최상단에 자동 배치됨
- Photoshop에서도 동일하게 사용 가능 (파일 > 스크립트 > 찾아보기 → JSX 파일 선택)

### 전체 기능 사용 (텍스트 제거 + 띄어쓰기 교정 + PSD/JSON 동시 출력)

인페인팅과 띄어쓰기 교정은 기본으로 켜져 있으므로, 별도 옵션 없이 아래 명령만으로 전체 기능이 동작함.
...
> 인페인팅 또는 띄어쓰기 교정을 끄려면 `--no-inpaint`, `--no-correct-spacing` 옵션을 사용.
...
| `--inpaint` / `--no-inpaint` | 켜짐 | 텍스트 제거 (인페인팅). `--no-inpaint`으로 비활성화 |
| `--correct-spacing` / `--no-correct-spacing` | 켜짐 | 한국어 띄어쓰기 교정. `--no-correct-spacing`으로 비활성화 |
...
[Text Layers]               ← 텍스트 레이어 그룹 (최상단)
  ...
  ├── Text 2: 대사내용...
  └── ...
[Inpainted (Text Removed)]  ← 텍스트 제거된 이미지 (--no-inpaint 사용 시 이 레이어 없음)
[Original Image]             ← 원본 이미지 (최하단)
```

- 각 텍스트 레이어는 원본 위치에 배치됨
- `--no-correct-spacing` 미사용 시 교정된 텍스트가 렌더링됨
- `--no-inpaint` 사용 시 중간 레이어(Inpainted) 없이 2단 구조로 출력됨
...
- 생성된 텍스트 그룹은 레이어 스택 최상단에 자동 배치됨
- 지정한 폰트가 없을 경우 Arial로 자동 대체됨
- `--no-correct-spacing` 미사용 시 교정된 텍스트가 적용됨

### 인페인팅 이미지 (PNG)

텍스트가 제거된 이미지가 `_inpainted.png` 파일로 별도 저장됨. PSD 내 Inpainted 레이어와 동일한 내용이며, 별도 활용 가능. `--no-inpaint` 사용 시 이 파일은 생성되지 않음.
...
4. **인페인팅** — AOT-GAN 기반. 텍스트 영역을 주변 배경으로 자연스럽게 채움 (`--no-inpaint`으로 비활성화 가능)
5. **띄어쓰기 교정** — py3langid로 언어 감지 후, 한국어 텍스트에만 kss 라이브러리로 맞춤법 기반 띄어쓰기 적용 (`--no-correct-spacing`으로 비활성화 가능)
